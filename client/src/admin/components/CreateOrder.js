  ...
  <div className="bg-white p-6 rounded-lg shadow-xl mb-6">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
      ...
      <div>
        ...
        <p className="text-lg text-gray-600 mb-4">Boxes: {product.boxes}</p>
        <p className="text-lg text-gray-600 mb-4">
          Bottles per Box: {product.bottlesPerBox || "N/A"}
        </p>
  ...
  const handleOrder = async () => {
    ...
    try {
      ...
      const payload = {
        products: validOrderProducts,
        paymentMethod: state.paymentMethod,
        deliveryChoice: state.isMiscellaneous
          ? "companyPickup"
      ...
      if (state.isMiscellaneous) localStorage.removeItem("token");
      toast.success(
        `Order created successfully: ${response.data.order.orderId}`
      );
    } catch (error) {
      const errorMessage =
        error.response?.data?.error ||
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {state.customer && (
          <div className="bg-white p-6 rounded-lg shadow-xl mb-6">
            <h3 className="text-xl font-semibold text-gray-800">
              {state.customer.name}{" "}
              {state.customer.firmName ? `(${state.customer.firmName})` : ""}
              {state.isMiscellaneous && " (Miscellaneous)"}
            </h3>
            <p className="text-gray-600">Email: {state.customer.email}</p>
            {state.customer.mobileNo && (
              <p className="text-gray-600">Mobile: {state.customer.mobileNo}</p>
            )}
        ...
        {state.orderStatus && (
          <p
            className={`mt-4 text-center ${
              state.orderStatus.includes("successfully")
                ? "text-green-600"
                : "text-red-600"
            }`}
          >
