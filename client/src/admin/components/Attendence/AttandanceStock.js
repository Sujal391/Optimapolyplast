...
const Attendance = () => {
  ...
  const handleCheckIn = async (e) => {
    ...
    try {
      ...
      setErrorCheckIn(err.response?.data?.error || "Error during check-in");
    } finally {
      setLoadingCheckIn(false);
    }
  };
  ...
  const handleCheckOut = async (e) => {
    ...
    try {
      ...
      setErrorCheckOut(err.response?.data?.error || "Error during check-out");
    } finally {
  ...
  return (
    <div className="container mx-auto p-6">
      ...
      <div className="flex flex-col md:flex-row justify-center items-start md:space-x-6 space-y-6 md:space-y-0">
        ...
        {/* --- Check-Out Form --- */}
        <div className="bg-white shadow-xl rounded-lg p-8 w-full md:w-96">
          <h2 className="text-2xl font-semibold text-center mb-6">Check-Out</h2>
          {errorCheckOut && (
