...
const DispatchComponent = () => {
  ...
  const updateCODPaymentStatus = async (orderId) => {
    try {
      const response = await api.patch(
        `/dispatch/orders/${orderId}/cod-payment-status`,
        {
      ...
      toast.success(
        response.data.message || "COD payment status updated successfully"
      );
      setProcessingOrders((prevOrders) =>
      ...
      toast.error(
        error.response?.data?.error || "Error updating COD payment status"
      );
    }
  };
  ...
  const generateChallan = async () => {
    try {
      const subtotal = challanData.items.reduce(
          ...
          <div className="bg-white p-6 rounded-2xl shadow-2xl w-full max-w-md transform transition-all duration-300 scale-95 hover:scale-100">
            ...
            <div className="mb-6">
              ...
              >
                ...
                <option value="payment_received_by_driver">
                  Received by Driver
                </option>
                ...
                <div key={field.label}>
                  ...
                  <input
                    ...
                    value={
                      field.value ||
                      challanData[
                        field.key || field.label.toLowerCase().replace(" ", "")
                      ]
                    }
                    onChange={(e) =>
                      !field.readOnly &&
          ...
          <div className="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto transform transition-all duration-300 scale-95 hover:scale-100">
            ...
            <div id="challan-content" className="space-y-6">
              <div className="text-center border-b pb-4">
                <img
                  src={logo}
                  alt="Optima Polyplast LLP Logo"
                  className="mx-auto w-38 h-32 mb-2"
                />
                <h1 className="text-2xl font-bold text-gray-800">
                  OPTIMA POLYPLAST LLP
                </h1>
                <p className="text-gray-600">
                  Plot No.12,296, Industrial Road, Near Umiya Battery
                </p>
                <p className="text-gray-600">
                  Mota Jalundra Industrial Zone, Derojnagar, Gandhinagar,
                  Gujarat
                </p>
                <p className="text-gray-600">
                  Phone: +919274658587 | Email: info@optimapoliplast.com
                </p>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <p>
                  <strong>Challan No:</strong> {generatedChallan.invoiceNo}
                </p>
                <p>
                  <strong>Date:</strong> {new Date().toLocaleDateString()}
                </p>
                <p>
                  <strong>User Code:</strong> {generatedChallan.userCode}
                </p>
                <p>
                  <strong>Receiver:</strong> {generatedChallan.receiverName}
                </p>
                <p>
                  <strong>Vehicle No:</strong> {generatedChallan.vehicleNo}
                </p>
                <p>
                  <strong>Mobile:</strong> {generatedChallan.mobileNo}
                </p>
                <p>
                  <strong>Driver:</strong> {generatedChallan.driverName}
                </p>
                <p>
                  <strong>Delivery Address:</strong>{" "}
                  {`${generatedChallan.shippingAddress.address}, ${generatedChallan.shippingAddress.city}, ${generatedChallan.shippingAddress.state} - ${generatedChallan.shippingAddress.pinCode}`}
                </p>
                <p>
                  <strong>Delivery Choice:</strong>{" "}
                  {generatedChallan.deliveryChoice === "homeDelivery"
                    ? "Home Delivery"
                    : "Company Pickup"}
                </p>
              </div>
              ...
              <div className="text-right space-y-2 text-sm">
                <p>
                  ...
                  {generatedChallan.subtotal.toFixed(2)}
                </p>
                <p>
                  <strong>GST (18%):</strong> ₹{" "}
                  {generatedChallan.gstAmount.toFixed(2)}
                </p>
                <p>
                  <strong>Delivery Charge:</strong> ₹{" "}
                  {generatedChallan.deliveryCharge === 0
                    ? "Free"
                    : generatedChallan.deliveryCharge.toFixed(2)}
                </p>
                <p className="text-lg font-semibold">
                  <strong>Grand Total:</strong> ₹{" "}
                  {generatedChallan.totalAmount.toFixed(2)}
                </p>
              </div>
              <div className="flex justify-between mt-6 pt-4 border-t text-sm">
                <p className="text-gray-600">
                  Issuer’s Signature: ________________
                </p>
                <p className="text-gray-600">
