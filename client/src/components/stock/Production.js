...
import { fetchRawMaterials, fetchProductionOutcomes, recordProductionOutcome } from '../../services/api/stock';
...
export default function Production() {
  ...
  const fetchData = async () => {
    ...
    setError('');
    try {
      const [materialsRes, outcomesRes] = await Promise.all([
        fetchRawMaterials(),
  ...
  const handleRecordProduction = async () => {
    if (!formData.rawMaterialId || !formData.usedQuantityKg || formData.outcomes.length === 0) {
    ...
    try {
      ...
      await recordProductionOutcome({
        ...
        usedQuantityKg: parseFloat(formData.usedQuantityKg),
        outcomes: formData.outcomes,
        wastageKg: formData.wastageKg ? parseFloat(formData.wastageKg) : 0,
        remarks: formData.remarks,
      });
      setSuccess('Production batch recorded successfully!');
      setFormData({
        rawMaterialId: '',
        usedQuantityKg: '',
        wastageKg: '',
    ...
    <div className="bg-gradient-to-r from-blue-50 to-blue-100 min-h-screen p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
          <h3 className="text-xl font-semibold text-gray-800 mb-4">Record Production Batch</h3>
          ...
          <div className="bg-white rounded-lg shadow-lg overflow-hidden">
                ...
                <tbody>
                  {paginatedOutcomes.map((outcome, index) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-gray-700">{outcome.usedQuantityKg} Kg</td>
            ...
            <div className="mt-4 flex items-center justify-between px-6 pb-6">
              <div className="text-sm text-gray-600">
                Showing {Math.min(outcomes.length, startIdx + 1)}–{Math.min(outcomes.length, endIdx)} of {outcomes.length}
              </div>
        ...
        {!loading && outcomes.length === 0 && (
          <div className="bg-white rounded-lg shadow-lg p-8 text-center">
            <p className="text-gray-500 text-lg">No production batches found. Record one to get started!</p>
          </div>
        )}
      </div>
    </div>
