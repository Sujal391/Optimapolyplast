...
import { fetchRawMaterials, recordCapProduction } from '../../../services/api/stock';
...
export default function CapProduction() {
  ...
  const handleSubmit = async () => {
    if (!formData.capType || !formData.quantityProduced || formData.rawMaterials.length === 0) {
      setError('Cap Type, Quantity Produced, and at least one Raw Material are required');
      return;
    }
    ...
    try {
      ...
      setSuccess('Cap production recorded successfully!');
