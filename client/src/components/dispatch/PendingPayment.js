              ...
              <tbody className="bg-white">
                {pendingPayments.map((order, index) => (
                  <tr key={index} className="border-b hover:bg-gray-50">
                    <td className="py-4 px-6 font-medium text-gray-800">{order.orderId}</td>
                    <td className="py-4 px-6">
                      <div className="font-semibold">{order.user.name}</div>
                      <div className="text-gray-900 text-xs">Firm: {order.user.firmName}</div>
                      <div className="text-gray-900 text-xs">User Code: {order.user.userCode}</div>
                      <div className="text-gray-900 text-xs">Phone: {order.user.phoneNumber}</div>
                      <div className="text-gray-900 text-xs">Email: {order.user.email}</div>
                    </td>
                    ...
                    <td className="py-4 px-6">
                      <ul className="list-disc list-inside space-y-1 text-gray-700">
                        {order.products.map((product, idx) => (
                          <li key={idx}>
                            {product.productName} ({product.productType}) × {product.boxes} @ ₹{product.price}
                          </li>
                        ))}
                      </ul>
                    </td>
                    <td className="py-4 px-6 font-semibold text-green-600">₹{order.totalAmount.toFixed(2)}</td>
                    <td className="py-4 px-6 font-semibold text-green-600">₹{order.deliveryCharge.toFixed(2)}</td>
                    <td className="py-4 px-6 font-bold text-indigo-700">₹{order.totalAmountWithDelivery.toFixed(2)}</td>
                    <td className="py-4 px-6">{order.paymentMethod}</td>
                    <td className="py-4 px-6">{order.orderStatus}</td>
                    <td className="py-4 px-6 text-red-500 font-semibold">{order.paymentStatus}</td>
                  </tr>
                ))}
              </tbody>
            </table>
